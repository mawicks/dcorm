...
def relations_inserted(tables_created):
    dcorm.set_connection_factory(lambda: tables_created)
...
def test_select_bar_join_foo_by_joining_select(relations_inserted):
    dcorm.set_connection_factory(lambda: relations_inserted)
    selected_foo = dcorm.select(Foo).where("Foo.a = 1")
    query = dcorm.select(Bar).join("some_foo", selected_foo)
    results = list(query())
    assert len(results) == 2


def test_select_bar_join_bar_on_some_foo_raises_type_error():
    selected_bar = dcorm.select(Bar).where("Bar.b = 1")
    # This should raise a type error because some_foo has type Foo
    # but we're joining it on a Select(Bar)
    with pytest.raises(TypeError):
        dcorm.select(Bar).join("some_foo", selected_bar)
